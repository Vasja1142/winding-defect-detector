...
Комплексная система компьютерного зрения для обнаружения дефектов в промышленных процессах намотки. Проект использует модель **YOLOv8** и представляет собой полный **MLOps-пайплайн**: от подготовки данных и обучения модели до развертывания в виде потокового real-time сервиса. Вся среда контейнеризирована с помощью **Docker** для обеспечения воспроизводимости и простоты развертывания.

![TODO: Добавить GIF с демонстрацией работы детектора]
...
- **Высокопроизводительная детекция:** Используется state-of-the-art модель YOLOv8 для точного и быстрого обнаружения объектов.
...
4.  **Обучение модели:** Скрипт `train.py` использует подготовленный датасет для обучения модели YOLOv8. Все результаты (веса, метрики) сохраняются в `data/05_runs/` и логируются в MLflow.
...
2.  **Подготовьте данные и предобученные модели:**
    ...
    - Скачайте предобученную модель YOLOv8 (например, `yolov8m.pt`) и поместите ее в `data/00_pretrained/`.
